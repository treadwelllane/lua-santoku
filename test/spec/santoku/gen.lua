...
test("santoku.gen", function ()
  ...
  test("cogen called as function (without step) returns val", function ()
  ...
  test("nkeys", function ()
    local v = vec(1, 2, nil, 3, nil, 4, nil, nil)
    local ks = gen.nkeys(v):vec()
    assert(ks.n == 8)
    for i = 1, ks.n do
      assert(ks[i] == i)
    end
  ...
  test("set", function ()
    local s = gen.pack("a", "b", "c"):set()
    local ks = gen.keys(s):vec():sort()
    local vs = gen.vals(s):vec()
    assert(ks.n == 3)
    assert(vs.n == 3)
    assert(ks[1] == "a")
    assert(ks[2] == "b")
  ...
  test("head", function ()
    assert(1 == gen.pack(1, 2, 3):co():head())
  end)

  test("includes", function ()
    assert(true == gen.pack(1, 2, 3):co():includes(2))
    assert(false == gen.pack(1, 2, 3):co():includes(4))
  end)
